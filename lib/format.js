import { MONTHS_FR, MONTHS_SHORT_FR, DEFAULT_CATEGORY_PALETTE } from "./constants";
...
// Couleur stable à partir d'un nom (toujours la même pour un nom donné), piochée dans la palette
// par défaut — utilisé pour les comptes (pas de couleur manuelle possible) et en repli pour les
// catégories sans couleur assignée.
export function paletteColor(name) {
  let hash = 0;
  for (let i = 0; i < name.length; i++) hash = (hash * 31 + name.charCodeAt(i)) >>> 0;
  return DEFAULT_CATEGORY_PALETTE[hash % DEFAULT_CATEGORY_PALETTE.length];
}

// Couleur d'une catégorie : celle assignée manuellement si elle existe, sinon une couleur de la
// palette par défaut choisie de façon stable à partir du nom (toujours la même pour un nom donné,
// même si l'ordre des catégories change).
...
// "Aujourd'hui" au sens du calendrier français, indépendamment du fuseau horaire du serveur (les
// fonctions Vercel tournent en UTC) — utilisée à la fois par la tâche planifiée des abonnements et
// par la génération immédiate à la création (voir lib/supabase.js), pour rester parfaitement cohérentes.
...
// Le chemin stocké en base garde la structure imbriquée d'origine (ex. "Fork and knife with
// plate/Color/xxx.svg") mais les fichiers réels sont servis à plat dans /category-icons/ — on ne
// garde que le nom de fichier.
export function categoryIconUrl(iconPath) {
