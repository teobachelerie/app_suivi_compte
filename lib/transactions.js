...
const SELECT_WITH_JOINS = "id, title, amount, date, type, payment_method, tags, splits, emoji, notes, categories(name), accounts!account_id(name), to_account:accounts!to_account_id(name)";
...
export async function createTransaction(userId, t) {
  const category_id = t.type === "Virement" ? null : await resolveId("categories", t.category, userId);
  ...
  const { data, error } = await supabase
    .from("transactions")
    .insert({ title: t.title, amount: toNumber(t.amount), date: t.date, type: t.type, category_id, account_id, to_account_id, payment_method: t.payment, user_id: userId, tags: t.tags || [], splits: t.splits || null, emoji: t.emoji || null, notes: t.notes || null })
...
export async function updateTransaction(userId, id, t) {
  ...
  if (t.category !== undefined) patch.category_id = t.type === "Virement" ? null : await resolveId("categories", t.category, userId);
...
export async function importTransactions(userId, rows) {
  ...
  // Un virement n'a plus besoin d'aucune catégorie (category_id reste tel que résolu, null pour
  // ces lignes puisque le type de transaction "Virement" suffit déjà à les distinguer).
