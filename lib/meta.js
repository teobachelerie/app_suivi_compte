...
import { DEFAULT_PAYMENTS, TIER_RANK } from "./constants";

export async function getSchema(userId) {
  const [{ data: categories, error: catErr }, { data: accounts, error: accErr }, plan] = await Promise.all([
    supabase.from("categories").select("id, name, color, parent_id, tier, icon, is_fallback").eq("archived", false).eq("user_id", userId).order("created_at", { ascending: true }),
  ...
  // Une catégorie sans palier (tier = null) est une catégorie "historique" créée avant la
  // taxonomie fixe — toujours visible pour son propriétaire, jamais restreinte. Une catégorie de
  // la taxonomie n'est visible que si son palier est couvert par celui de l'utilisateur.
  const visibleCategories = categories.filter((c) => !c.tier || TIER_RANK[c.tier] <= TIER_RANK[plan.tier]);

  // Structure hiérarchique pour le Raccourci iOS : la liste des familles, puis pour chaque
  // famille la liste de ses sous-catégories AVEC LA FAMILLE ELLE-MÊME EN PREMIER (choisir ce
  // premier élément = appliquer la famille seule, sans sous-catégorie précise) — exactement le
  // même principe que le sélecteur de catégorie de l'app.
  const families = visibleCategories.filter((c) => !c.parent_id);
  const subcategoriesByFamily = {};
  families.forEach((fam) => {
    const subs = visibleCategories.filter((c) => c.parent_id === fam.id).map((c) => c.name);
    subcategoriesByFamily[fam.name] = [fam.name, ...subs].join(",");
  });

  return {
    categories: visibleCategories, accounts, payments: DEFAULT_PAYMENTS,
    ...
    categoryNames: visibleCategories.map((c) => c.name).join(","),
    categoryFamilies: families.map((c) => c.name).join(","),
    categorySubcategoriesByFamily: JSON.stringify(subcategoriesByFamily),
