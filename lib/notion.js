...
function transactionToProperties(t) {
  ...
  if (t.amount !== undefined) props.Amount = { number: toNumber(t.amount) };
...
export async function getSchema() {
  ...
  return {
    categories: (data.properties.Category?.select?.options || []).map((o) => ({ id: o.id, name: o.name })),
    accounts: (data.properties.Compte?.select?.options || []).map((o) => ({ id: o.id, name: o.name })),
    payments: (data.properties["Payment Method"]?.select?.options || []).map((o) => o.name),
  };
}

// `options`: tableau de { id?, name }. Un id transmis met à jour l'option Notion existante
// en place (donc la renomme, y compris sur toutes les transactions déjà associées à cette
// option puisque Notion résout l'affichage par id, pas par nom). Sans id, Notion crée une
// nouvelle option. Une option absente du tableau est retirée de la liste des choix futurs
// mais reste affichée telle quelle sur les transactions qui la portaient déjà.
export async function updateSelectOptions(propertyName, options) {
  const res = await fetch(`${BASE}/databases/${process.env.NOTION_DATABASE_ID}`, {
    ...
    headers: headers(),
    body: JSON.stringify({
      properties: { [propertyName]: { select: { options: options.map((o) => (o.id ? { id: o.id, name: o.name } : { name: o.name })) } } },
    }),
  });
  const data = await res.json();
