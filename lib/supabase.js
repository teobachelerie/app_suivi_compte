...
export async function getUserPlan(userId) {
  const { data, error } = await supabase.from("user_plans").select("tier, stripe_subscription_status, current_period_end").eq("user_id", userId).maybeSingle();
  if (error) throw new Error(error.message);
  ...
  return { tier, limits: TIER_LIMITS[tier], stripeStatus: data?.stripe_subscription_status || null, currentPeriodEnd: data?.current_period_end || null };
}
...
export async function setUserPlanFromCheckout(userId, tier, customerId, subscriptionId, status, currentPeriodEnd) {
  const { error } = await supabase.from("user_plans").update({
    ...
    stripe_subscription_status: status, current_period_end: currentPeriodEnd, updated_at: new Date().toISOString(),
  }).eq("user_id", userId);
  if (error) throw new Error(error.message);
}
